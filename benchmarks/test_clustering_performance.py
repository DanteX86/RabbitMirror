...
class TestClusteringPerformance:
    ...
    def test_clustering_scalability(self):
        ...
        results = scalability_test(
            cluster_data, data_sizes, lambda size: size, iterations=3
        )
            ...
            for min_samples in min_samples_values:
                ...
                for _ in range(3):
                    try:
                        ...
                        print(
                            f"Error with parameters eps={eps}, "
    ...
    def test_clustering_algorithms_comparison(self):
        ...
        results = compare_implementations(
            *implementations, test_data=test_data["entries"], iterations=5
        )
        ...
        for impl_name, impl_func in implementations:
            try:
                ...
                results[impl_name].update(
                    {
                        "total_clusters": cluster_result["cluster_info"][
                            "total_clusters"
                        ],
                        "noise_points": cluster_result["cluster_info"]["noise_points"],
                        "clustered_entries": cluster_result["cluster_info"][
                            "total_entries"
                        ]
                        - cluster_result["cluster_info"]["noise_points"],
                        "clustering_ratio": (
                            cluster_result["cluster_info"]["total_entries"]
                            - cluster_result["cluster_info"]["noise_points"]
                        )
                        / cluster_result["cluster_info"]["total_entries"],
                    }
    ...
    def test_clustering_memory_efficiency(self):
        ...
        for size in data_sizes:
            ...
            results[f"size_{size}"] = {
                ...
                "clustering_efficiency": cluster_result["cluster_info"][
                    "total_clusters"
                ]
    ...
    def test_clustering_concurrent_performance(self):
        ...
        results = {
            "sequential_clustering": {
                ...
                "total_clusters": sum(
                    r["cluster_info"]["total_clusters"] for r in sequential_results
                ),
            },
            "concurrent_clustering": {
                ...
                "total_entries": sum(len(d) for d in datasets),
                "total_clusters": sum(
                    r["cluster_info"]["total_clusters"] for r in concurrent_results
                ),

...
WEB_HOST ?= 127.0.0.1
WEB_PORT ?= 5001
WEB_LOG  ?= /tmp/rabbitmirror_web.log
WEB_PID  ?= /tmp/rabbitmirror_web.pid

.PHONY: web-start web-stop web-restart
web-start: ensure-venv ## Start the RabbitMirror Flask web app in background
	@echo "Starting RabbitMirror web on $(WEB_HOST):$(WEB_PORT)..."
	@if [ -f "$(WEB_PID)" ] && ps -p "$$(cat $(WEB_PID))" > /dev/null 2>&1; then \
		echo "Already running (PID $$(cat $(WEB_PID))). Stop it with 'make web-stop'."; \
		exit 0; \
	fi
	@env FLASK_HOST=$(WEB_HOST) FLASK_PORT=$(WEB_PORT) SECRET_KEY="dev" \
	  nohup "$(VENV_BIN)/python" -m rabbitmirror.web.app \
	  1>"$(WEB_LOG)" 2>&1 & echo $$! > "$(WEB_PID)"
	@sleep 1
	@lsof -nP -iTCP:$(WEB_PORT) -sTCP:LISTEN | awk 'NR>1 {print "Listening:", $$0}' || (echo "Failed to start. See $(WEB_LOG)"; exit 1)
	@echo "PID $$(cat $(WEB_PID)). Logs: $(WEB_LOG)"

web-stop: ## Stop the Flask web app if running
	@PID=""; if [ -f "$(WEB_PID)" ]; then PID=$$(cat "$(WEB_PID)"); fi; \
	if [ -n "$$PID" ] && ps -p $$PID > /dev/null 2>&1; then \
		echo "Stopping PID $$PID..."; \
		kill $$PID || true; \
		for i in 1 2 3 4 5; do if ps -p $$PID > /dev/null 2>&1; then sleep 0.5; else break; fi; done; \
	...
	rm -f "$(WEB_PID)"; \
	if lsof -nP -iTCP:$(WEB_PORT) -sTCP:LISTEN > /dev/null; then \
		DET_PID=$$(lsof -nP -iTCP:$(WEB_PORT) -sTCP:LISTEN -t | head -n1); \
		if [ -n "$$DET_PID" ]; then echo "Stopping detected listener PID $$DET_PID on port $(WEB_PORT)..."; kill $$DET_PID || true; fi; \
	fi; \
	echo "Ensuring port $(WEB_PORT) free..."; \
	if lsof -nP -iTCP:$(WEB_PORT) -sTCP:LISTEN > /dev/null; then lsof -nP -iTCP:$(WEB_PORT) -sTCP:LISTEN; echo "Warning: something is still listening on $(WEB_PORT)."; else echo "Port $(WEB_PORT) is free."; fi

web-restart: web-stop web-start ## Restart the Flask web app

# ------------------------
# API demo convenience
# ------------------------
API_BASE ?= http://localhost:5001
API_KEY ?=

.PHONY: api-analyze api-analyze-raw api-demo
api-analyze: ## POST sample payload to /api/analyze and pretty-print the result
	@echo "Posting sample payload to $(API_BASE)/api/analyze"
	@HDR_API_KEY=""; \
	if [ -n "$(API_KEY)" ]; then HDR_API_KEY="-H 'X-API-Key: $(API_KEY)'"; fi; \
	CMD="curl -sS -H 'Content-Type: application/json' $$HDR_API_KEY --data-binary @examples/api/sample_watch_history.json $(API_BASE)/api/analyze"; \
	sh -c "$$CMD | jq ." 2>/dev/null || sh -c "$$CMD | '$(VENV_BIN)/python' -m json.tool"

api-analyze-raw: ## POST sample payload to /api/analyze and print raw JSON
	@echo "Posting sample payload to $(API_BASE)/api/analyze (raw)"
	@HDR_API_KEY=""; \
	if [ -n "$(API_KEY)" ]; then HDR_API_KEY="-H 'X-API-Key: $(API_KEY)'"; fi; \
	curl -sS -H 'Content-Type: application/json' $$HDR_API_KEY --data-binary @examples/api/sample_watch_history.json $(API_BASE)/api/analyze

api-demo: ## Start web app, call api-analyze, then stop web app
	@$(MAKE) web-start WEB_HOST=$(WEB_HOST) WEB_PORT=$(WEB_PORT)
	@$(MAKE) api-analyze API_BASE=http://$(WEB_HOST):$(WEB_PORT) API_KEY=$(API_KEY)
	@$(MAKE) web-stop WEB_PORT=$(WEB_PORT)
